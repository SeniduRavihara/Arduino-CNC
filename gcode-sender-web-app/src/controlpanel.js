...
app.controller(
  "controlPanelCtrl",
  function ($scope, $timeout, hotkeys, settingsService, machineService) {
    ...
    // Use a direct step size value, default to 10
    $scope.stepSize = 10;
    ...
    $("#input-control-cmd").keydown(function (e) {
      ...
      if (e.keyCode == 27) {
        ...
        $timeout(function () {
          $("#input-control-cmd")[0].setSelectionRange(
            prevCommand.length,
            prevCommand.length
          );
        }, 1);
      } else if (e.keyCode == 40) {
        // down arrow; show next history position.
        manualInputPosition = Math.min(
          manualInputPosition + 1,
          manualInputHistory.length
        );
        var nextCommand =
          manualInputPosition < manualInputHistory.length
            ? manualInputHistory[manualInputPosition]
    ...
    var shouldSendCommands = function () {
      return (
        machineService.isConnected && machineService.commandQueue.length == 0
      );
    };
