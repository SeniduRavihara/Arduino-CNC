...
app.service("machineService", function ($rootScope, $timeout, warningService) {
  ...
  var connect = async function (port, baud) {
    api.isBusy = true;
    try {
      // Request a port if not provided
      if (!port) {
        serialPort = await navigator.serial.requestPort();
      } else {
        serialPort = port; // If port is provided (advanced usage)
      }
      ...
      // Start reading loop
      readSerialLoop();
      // (Optional) Wake Lock API can be used here for power management
    } catch (err) {
      warningService.warn("connection", "Unable to connect: " + err);
      api.isBusy = false;
  ...
  var disconnect = async function () {
    // (Optional) Release Wake Lock here if used
    api.commandQueue = [];
    if (serialReader) {
      keepReading = false;
      try {
   ...
   */
  var sendCommandToSerialConnection = async function (cmd) {
    ...
    try {
      ...
      warningService.warn("cmd", "failed to send command: " + err);
    }
  };
  ...
  var processCommandQueue = function () {
    ...
    if (api.commandQueue.length === 0 || !serialPort || api.pendingAck) {
      return;
    }
  ...
  async function readSerialLoop() {
    while (keepReading && serialReader) {
      try {
        ...
        if (keepReading) warningService.warn("serial", "Read error: " + err);
        break;
      }
    }
  }
